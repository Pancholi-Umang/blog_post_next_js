  ...
  return (
    <Container className="ps-5 pe-5">
      ...
      <Row className={`d-flex align-items-center justify-content-center flex-column mt-5 ${styles.colorOfHeader}`}>
        <Col lg={10} sm={12}> <h3 className="ps-3">Food</h3> <hr /> </Col>
      </Row>
      <Row className="d-flex align-items-center justify-content-center flex-column">
        <Col lg={10} sm={12} className="d-flex justify-content-center flex-column align-items-center">
          {fetchFood?.map((foodValue) => {
            return (
              <div key={foodValue?.id} onClick={() => router.push(`food/${foodValue?.id}`)} className="my-2" style={{ cursor: "pointer" }}>
                <div key={foodValue?.id} className="text-d-none">
                  <img src={foodValue?.image} alt="foodImage" className="img-fluid w-100" />
