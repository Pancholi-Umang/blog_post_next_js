...
const Restorent = () => {
  ...
  useEffect(() => {
    const url = "http://localhost:3000/api/best_restorent";

    const fetchData = async () => {
      try {
        const response = await axios.get(url);
        setRestorent(response?.data);
      } catch (error) {
  ...
  return (
    <Container className="ps-5 pe-5">
      <Row className={`d-flex align-items-center justify-content-center flex-column mt-5 ${styles.colorOfHeader}`}>
        <Col lg={10} sm={12}> <h3 className="ps-3">Restorents</h3> <hr /> </Col>
      </Row>
      <Row className="d-flex align-items-center justify-content-center flex-column">
        <Col lg={10} sm={12}>
          {
            restorent?.map((foodValue) => {
              return (
